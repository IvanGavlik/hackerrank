 ...
 * 13.09.2017
 * Explanation 0
 * In the year y = 2017, January has 31 days, February has 28 days, March has 31 days, April has 30 days, May has 31
 * days, June has 30 days, July has 31 days, and August has 31 days. When we sum the total number of days in the first
 * eight months, we get 31 + 28 + 31 + 30 + 31 + 30 + 31 + 31 = 243. Day of the Programmer is the 256th day, so then
 * calculate 256 - 243 = 13 to determine that it falls on day 13 of the 9th month (September).
 ...
 * Day of the Programmer is the 256th day, so then calculate 256 - 244 = 12 to determine that it falls on day 12 of
 * the 9th month (September). We then print the full date in the specified format, which is 12.09.2016.
 *
 * Sample Input 2
 * 1800
 * Sample Output 2
...
public class ProgrammerDay {
    ...
    private static boolean isJulianCalendarLeap(int year) {
        return year % 4 == 0;
    }

    private static boolean isGregorianCalendar(int year) {
        return year >= 1919 && year <= 2700;
    }

    private static boolean isGregorianCalendarLeap(int year) {
